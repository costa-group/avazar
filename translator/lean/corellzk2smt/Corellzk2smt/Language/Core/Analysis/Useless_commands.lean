...
/- This module implements dead-code elimination for the core language, driven by liveness
   analysis. Liveness information is (re)computed and stored in the metadata of each command
   and function as a side effect, but commands whose result is never live are discarded.

   The main function is `removeUselessProg` which takes a program and returns a new program
   with useless commands removed from every function and liveness information added to what
   remains. Liveness (and hence usefulness) of each function's commands is computed by
   traversing them in reverse order and keeping track of the live variables at each point.

   Traversing in reverse order is done for now with recursion, we should consider
   reversing the list and use tail-recursion.
-/

namespace Corellzk2smt.Language.Core.Analysis.Useless_commands
...
def liveInOfBody {c : ZKConfig} (body : List (ComWithMD c)) (out : VarIDSet) : VarIDSet :=
    getCmdsLiveIn (Corellzk2smt.Language.Core.Analysis.Liveness.addLivenessCmds body out) out
...
/- Computes the liveness information for a single command and adds it to the meta data,
   or returns `none` if the command is useless, i.e. if none of the values it produces
   (the variable it assigns/writes/returns) are in the `out` live-out set. -/
...
/-- Computes the liveness information for a function, adds it to the
    meta data, and removes useless commands -/
...
/-- Computes the liveness information for a program and removes useless
    commands -/
...
end Corellzk2smt.Language.Core.Analysis.Useless_commands
