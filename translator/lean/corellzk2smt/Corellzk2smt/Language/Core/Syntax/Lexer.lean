...
def TokenInfo.toString (info : TokenInfo) : String :=
  s!"{info.token} at ({info.row},{info.col})"
...
/- Main Tokenizer:
   ...
   Converts the char-stream into a token.
...
partial def Lexer.nextToken (st : Lexer) : IO (TokenInfo × Lexer) := do
  ...
  let row := st.row
    ...
    -- Equality token: '==' (or '=' when the 2nd '=' is not present)
    else if c == '=' then
      let (_, st) ← st.next -- consume the '=' character
    ...
    -- Arrow sign: '->' (or '-' when the '>' is not present)
    else if c == '-' then
      let (_, st) ← st.next -- consume the '-' character
      let (nextCharOpt, st) ← st.peek
      if nextCharOpt == some '>' then
    ...
    -- Identifier: any sequence of alphanumeric characters, '_', '%', '@', '#', or '.'
    -- that starts with a letter, underscore, '%', or '@'.
    -- We do not allow the identifier to be a single character '_', '%', '@', '#', or '.'.
    else if c == '_' || c == '%' || c == '@' || c == '.' || c.isAlpha then
      let (s, st) ← st.scanWhile (fun x => x.isAlphanum || x == '_' || x == '%' || x == '@' || x == '#' || x == '.')
      if s.length == 1  && !c.isAlpha then
        throw <| .userError s!"Lexer error: invalid identifier '{s}' at ({row},{col})"
      else
...
end Corellzk2smt.Language.Core.Syntax.Lexer
