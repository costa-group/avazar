...
/- Correctness of `seFuncCall`, for the FF-only case (every parameter/return of the callee is
   `.ff`-typed, no arrays) -- see the session's design discussion in project memory
   (`project-corellzk2smt-seFunc-design`) for why the array-flattening case is deferred, and why
   this needs a `H_specCorrect` hypothesis (a stand-in for what `seFunc`'s own correctness
   theorem will eventually discharge for real) rather than being provable outright. -/
...
/-- Setting several fresh/matching values into a symbolic and a concrete environment
    ...
    preserves `EnvMatches`, given the values being set already match pointwise and the id list
    has no repeats (so "last write wins" picks the same entry on both sides). -/
theorem EnvMatches_setVars {c : ZKConfig} :
  ...
  induction ids with
  | nil =>
      intro svs vs assignment symEnv env hmatch _hforall _hnodup symEnv' hsym env' henv
      cases svs with
      | nil =>
          cases vs with
          | nil =>
  ...
  | cons id ids ih =>
      intro svs vs assignment symEnv env hmatch hforall hnodup symEnv' hsym env' henv
      cases svs with
      ...
      | cons sv svs' =>
        ...
        | cons v vs' =>
          ...
          obtain ⟨hid_notmem, hids_nodup⟩ := List.nodup_cons.mp hnodup
          ...
          | ok symEnvMid =>
              ...
              | ok envMid =>
                  ...
                  exact ih svs' vs' assignment (Corellzk2smt.SymExec.Basic.setVar symEnv id sv)
                    ...
                    hrest_forall hids_nodup symEnvMid hsym' envMid henv'
...
/-- Correctness of `seFuncCall` at a call site, now array-general: the callee's params/rets may
    be `.array`-typed (given (a) the callee's `FuncSpec` and its own per-function correctness
    (`H_specCorrect`, a stand-in for what `seFunc`'s own correctness theorem will eventually
    discharge for real -- see `project-corellzk2smt-seFunc-design`), and (b) call-site
    well-formedness (arity matches the callee's signature, args always concretely defined *and
    well-typed against it* via `ValuesMatchParams`, `outs` has no repeats)). `houts_nodup` is a
    proof-simplifying restriction, not a semantic requirement (duplicate `outs` are harmless --
    both sides already agree on overwrite-last-wins semantics), flagged as such rather than
    folded in silently. -/
theorem seFuncCall_correct {c : ZKConfig} (gconf : GlobalConfig c) (p : Prog c)
  ...
  obtain ⟨argSymVals, hargSV_len, hargSV_forall, hargSV_eq⟩ :
    ...
    exact resolveSimpleExprToSymValue_defined_of_evalSimpleExprToValue symEnv s _ default hmatch v hv
        ...
        | ok vars_env =>
            ...
            have hEnvMatchesOut : EnvMatches assignment' outSymEnv' vars_env :=
              EnvMatches_setVars outs outVals retVals assignment' symEnv env
                hEnvMatchesMid hOutValsMatch houts_nodup outSymEnv' hOutSymEnv vars_env hSV
  ...
  have hcomplete : ∀ (env : Env c) (assignment : Assignment c),
    ...
    · have hEnvMatchesMid : EnvMatches assignment' symEnv env :=
      ...
      exact EnvMatches_setVars outs outVals retVals assignment' symEnv env
        hEnvMatchesMid hOutValsMatch houts_nodup outSymEnv' hOutSymEnv vars_env hSV
  refine ⟨_, hseFuncCall_eq, rfl, hmono, hfresh, hbelowProof, houtbelow, houtfresh, hsound,
    hcomplete⟩
