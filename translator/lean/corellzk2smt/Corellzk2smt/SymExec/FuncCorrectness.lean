...
/- Correctness of `seFunc`, for the FF-only case (every parameter/return of the function being
   translated is `.ff`-typed, no arrays -- `seFunc` itself already errors out on arrays, so this
   restriction is enforced by construction, not by a side hypothesis). Mirrors
   `SymExec/FuncCallCorrectness.lean`'s scope decision for `seFuncCall_correct`.

   `seFunc_correct` takes `H_simple`/`H_funcCall`/`H_domain`/`H_shape` as assumed hypotheses,
   exactly like `seCmd_correct`/`seCmds_correct` do -- not because this theorem needs to
   discharge them, but because it calls `seCmds_correct` on the function's own body, and
   `seSimpleCmd` remains a stub. Discharging `H_funcCall` for a whole program (by induction
   over the function list, using this theorem's conclusion for earlier functions) is
   `seExecFuncs_correct`'s job, not this one's. -/

namespace Corellzk2smt.SymExec.FuncCorrectness
          ...
          | ok result2 =>
            ...
            have hinSymEnv_below : varSetBelow (symEnvVars inSymEnv) nv1 :=
              mintFreshParams_symEnvVars_below 0 params _ (zeroSymEnv_below _ 0) nv1 paramVars
                inSymEnv hmp
            obtain ⟨bodySpec', hbodySpec'_eq, hbs_in, hbs_mono, hbs_fresh, hbs_below,
              hbs_outbelow, hbs_outfresh, hbs_sound, hbs_complete⟩ :=
              seCmds_correct gconf p' specs H_simple H_funcCall H_domain H_shape
                { nextVarId := nv1 } body hshaped inSymEnv hinSymEnv_below
            have hbodySpecEq : bodySpec = bodySpec' := by
              have heq := hbs.symm.trans hbodySpec'_eq
...
/-- Single-function wrapper connecting `seFunc_correct` to `seFuncCall_correct`: given a
    function `func` (fetched from `p` at `fname`) that `seFunc` translates to `fspec` using
    `specs` (the specs of functions callable from within `func`'s own body -- i.e. everything
    strictly earlier in the program, per the "functions can only call earlier functions"
    invariant), `seFuncCall`'s own `TranslatesCorrectly` conclusion holds for a *caller* whose
    spec list is `fspec :: specs` (`fspec` prepended to the exact same list `seFunc` used) --
    matching the natural invariant of how a whole program's spec list would actually be built
    (each function's spec computed over, then prepended in front of, the specs of everything
    before it). Discharges `seFuncCall_correct`'s `hspec_retsShape`/`H_specCorrect`/
    `hspec_paramsFF`/`hspec_retsFF`/`hspec_eq` hypotheses via `seFunc_correct`/`seFunc_eq_shape`/
    `fetchFunc_name_eq` instead of assuming them; the abstract `H_simple`/`H_funcCall`/
    `H_domain`/`H_shape` hypotheses `seFunc_correct` itself still needs remain as hypotheses here
    too (discharging those for a whole program is `seExecFuncs_correct`'s job, not this one's). -/
...
end Corellzk2smt.SymExec.FuncCorrectness
