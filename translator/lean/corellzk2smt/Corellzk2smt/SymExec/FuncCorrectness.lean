...
/- Shared machinery for `seFunc` correctness (array-general -- function params/rets may be
   `.array`-typed): the `mintFreshParam`/`mintFreshParams`/`mintFreshRetsWithEq` families,
   `EnvMatches_mintFreshParams_bindInParams_general`, `getOutParamsValues_construct_general`, and
   their many supporting lemmas, plus `fetchFunc_name_eq`/`seFunc_eq_shape` (used to connect
   `seFunc_correct` to `seFuncCall_correct` at a call site). None of this is stated in terms of
   ...
   `SymExec/PartialCorrectness/FuncCorrectness.lean` -- see that file's header for the actual
   `seFunc_correct`/`seFuncCall_correct_via_seFunc` theorems (still take `H_simple`/`H_funcCall`/
   `H_domain`/`H_shape` as assumed hypotheses, since `seFunc_correct` calls `seCmds_correct` on
   the function's own body, and `seSimpleCmd` remains a stub) built on top of this. -/
...
theorem seFunc_f_params_split {c : ZKConfig} (gconf : GlobalConfig c) (specs : List (FuncSpec c))
    (name : FName) (params rets : List Param) (body : List (ComWithMD c)) (fspec : FuncSpec c)
    (h : seFunc gconf specs (Func.mk name params rets body) = Except.ok fspec) :
