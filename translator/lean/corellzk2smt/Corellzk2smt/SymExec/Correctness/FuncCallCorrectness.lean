...
conditional one fully superseded it, leaving the split purely historical -- merged back into one
file/namespace now that there's only one consumer.

Partial-correctness analogue of `seFuncCall_correct`: its whole proof body (deriving
`hseFuncCall_eq`, `hmono`, `hfresh`, `hbelowProof`, `houtbelow`, `houtfresh`, `hsound`,
`hcomplete`) is *identical* to the old, unconditional version -- none of it depends on whether the
surrounding `TranslatesCorrectly` is conditional or not, it's all just facts about what
`seFuncCall` computes. The only change from the unconditional version is at the very start
(`intro symEnv hbelow spec hspec_witness` instead of `intro symEnv hbelow`, taking the produced
`spec` and success witness as given) and the very end (deriving `spec = {...}` via
