...
/-- `seEvalExpr` on `.bop .bor s1 s2`, when it succeeds, does so via `seExprBor`'s exact defining
    shape -- the condition "both operands are `0`" tied to the fresh var via a term-level `ite`,
    plus the same `bool_ffterm` tag conjunct as `seExprEq`/`seExprNeq`. Stated directly against the
    implementation, same reason as `seEvalExpr_eq_eq`/`seEvalExpr_neq_eq`. -/
...
/-- Mirror of `seEvalExpr_bor_eq`, for `.band` -- condition "either operand is `0`" instead of
    "both", same `bool_ffterm` tag conjunct. -/
theorem seEvalExpr_band_eq {c : ZKConfig} (md : CmdMD) (gconf : GlobalConfig c)
    (sconf : SymExecConfig c) (symEnv : SymEnv c) (specs : List (FuncSpec c))
    (s1 s2 : SimpleExpr c) (exprSpec : ExprSpec c)
    (heq : seEvalExpr md gconf sconf symEnv specs (Expr.bop BinOp.band s1 s2)
      = Except.ok exprSpec) :
    ∃ v1 v2 fbool, resolveSimpleExpr symEnv s1 = Except.ok v1 ∧
      resolveSimpleExpr symEnv s2 = Except.ok v2 ∧
      bool_ffterm gconf sconf (FFTerm.var sconf.nextVarId) = Except.ok fbool ∧
      ...
      exprSpec.f = FFFormula.and
        (FFFormula.eq (FFTerm.var sconf.nextVarId)
          (FFTerm.ite
            ...
            (FFTerm.val 0) (FFTerm.val 1)))
        fbool := by
  simp only [seEvalExpr, seExprBAnd] at heq
  cases hres1 : resolveSimpleExpr symEnv s1 with
  | error msg => rw [hres1] at heq; simp at heq
  | ok v1 =>
      rw [hres1] at heq
...
/-- `seEvalExpr` on `.uop .bneg s`, when it succeeds, does so via `seExprBNeg`'s exact defining
    shape -- the condition "operand is `0`" tied to the fresh var via a term-level `ite`, plus the
    same `bool_ffterm` tag conjunct as `seExprEq`/`seExprNeq`/`seExprBor`/`seExprBAnd`. Stated
    directly against the implementation, same reason as `seEvalExpr_neg_eq`. -/
theorem seEvalExpr_bneg_eq {c : ZKConfig} (md : CmdMD) (gconf : GlobalConfig c)
    (sconf : SymExecConfig c) (symEnv : SymEnv c) (specs : List (FuncSpec c))
    (s : SimpleExpr c) (exprSpec : ExprSpec c)
    (heq : seEvalExpr md gconf sconf symEnv specs (Expr.uop UnOp.bneg s) = Except.ok exprSpec) :
    ∃ v fbool, resolveSimpleExpr symEnv s = Except.ok v ∧
      bool_ffterm gconf sconf (FFTerm.var sconf.nextVarId) = Except.ok fbool ∧
      exprSpec.outSymEnv = symEnv ∧
      exprSpec.f = FFFormula.and
        (FFFormula.eq (FFTerm.var sconf.nextVarId)
          (FFTerm.ite (FFFormula.eq (simpleSymValToTerm v) (FFTerm.val 0))
            (FFTerm.val 1) (FFTerm.val 0)))
        fbool := by
  simp only [seEvalExpr, seExprBNeg] at heq
  ...
  | ok v =>
      ...
      | ok fbool =>
          ...
          injection heq with heq
          subst heq
          exact ⟨v, fbool, rfl, rfl, rfl, rfl⟩

/-- `seEvalExpr` never succeeds on `.uop .not` today -- `seExprBitwiseNOT` is still a permanent
