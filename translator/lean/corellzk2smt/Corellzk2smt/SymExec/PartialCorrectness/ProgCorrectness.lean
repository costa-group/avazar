...
import Corellzk2smt.SymExec.ProgCorrectness

/-
Partial-correctness analogue of `Corellzk2smt.SymExec.ProgCorrectness`: whole-program
correctness for `seExecFuncs`/`seExecProg`, built on the conditional `seFunc_correct`/
`seFuncCall_correct_via_seFunc` (`PartialCorrectness/FuncCorrectness.lean`).
...
3. `H_domain` and `H_shape` no longer exist as parameters anywhere in this whole chain -- both
   ...
   - `H_domain`'s replacement, `seCmds_domain_of_defined`/`seCmd_domain_of_defined`/
     ...
     recursion. `seFunc_correct` (`PartialCorrectness/FuncCorrectness.lean`) discharges the
     premise itself, using `zeroSymEnv_contains`/`mintFreshParams_contains_mono`
     (`SymExec/FuncCorrectness.lean`) to show its own `inSymEnv` construction already satisfies
     it -- so nothing needs threading any further up than that, and `H_domain` is gone by the
     time whole-program level (this file) is reached.
...
open Corellzk2smt.SymExec.ProgCorrectness
