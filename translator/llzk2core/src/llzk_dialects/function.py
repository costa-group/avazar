...
class FunctionCall(Operation):
    ...
    def to_core(self, ctx: TranslationContext) -> str:
        ...
        # The out args are needed to generate the name "result_outarg"
        # to assign all the possible results
        ...
        # The variables assigned to the result are generated by adding to the result
        # variable the corresponding out_arg representation
        yield f"call {core_func}({args}) to {','.join(SSAVar(result.name + '_' + out_arg).to_core() for out_arg, _ in out_args)}"
