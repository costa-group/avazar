...
## 46. `scf.while` trip-count simulation raised a raw `KeyError` when the loop-carried variable's own recurrence directly reused a `felt.const` hoisted above the while

**Reported:** `poseidon3_new_optimized.mlir` failed translation with `KeyError: '%felt_const_1'` inside `construct_function_from_expressions` (`core_utils.py`), with the user's own suspicion that "there could be a variable that is missing".

**Root cause:** traced to `MixS_9::compute`'s first `scf.while` (`%1:2 = scf.while (%arg1 = %felt_const_0, %arg2 = %felt_const_0) ...`, condition `bool.cmp lt(%arg1, %felt_const_3)`). Every sibling `scf.while` in this file redefines its own local increment constant inside the body (e.g. `%felt_const_1_0 = felt.const 1` right before the `felt.add`) — this one instead reuses `%felt_const_1`, a constant already in scope, defined just *above* the while (`%11 = felt.add %arg1, %felt_const_1`). Valid SSA, but `scf.py`'s `SCFWhile._build_while_var_expressions`/`_process_while_variables` only scans operations inside the while's own `before_body`/`after_body` when building `var2expression`, so an operand defined outside the while never gets an entry. `_resolve_comparison_recurrence` already handled exactly this situation for the condition's *bound* (free-variable resolution via `ctx.var2const`, §45's neighborhood) — but called `construct_function_from_expressions` on the loop-carried `variable` itself one step earlier, with no equivalent resolution, so any external free variable reached while walking the variable's own recurrence chain crashed with a raw, uncaught `KeyError`.

**Fix:** `_resolve_comparison_recurrence` (`core_utils.py`) now runs the same free-variable-resolution routine already used for `bound` against `variable` first: `_collect_free_var_names(variable, var2expression, set())`, folding each name present in `ctx.var2const` into `var2expression` as a `FeltConst` leaf (identical convention to the existing bound-resolution loop). Unlike the bound (which falls back to a `SymbolicSteps` formula when unresolved), the loop-carried variable's own recurrence must be fully concrete to simulate at all — there's no symbolic-update equivalent — so any of `variable`'s free vars that remain unresolved now raise a clear `NotImplementedError` naming the variable and the unresolved names, instead of a raw `KeyError`. No signature changes to `construct_function_from_expressions`, `_collect_free_var_names`, or any caller; `_infer_from_comparison` and `_infer_sequence_from_comparison` both go through `_resolve_comparison_recurrence`, so the trip-count and index-sequence paths are fixed together.

**Verified:** two new tests in `TestInferNRepetitions` (`tests/test_core_utils.py`) mirroring the existing bound free-variable test, but for the variable's own recurrence: one resolving the external constant via `var2const` (asserts the correct trip count), one where it isn't in `var2const` either (asserts `NotImplementedError`, not `KeyError`). Full suite: 570 tests, up from 568 (the same 8 pre-existing `test_signal_renaming.py` failures as always, caused by the user's own uncommitted debug scaffolding, unrelated — confirmed unchanged by stashing this fix and re-running). End-to-end: confirmed the fix is load-bearing by reverting `core_utils.py` only and re-running the real `poseidon3_new_optimized.mlir` translation, which reproduced the exact original `KeyError: '%felt_const_1'`; with the fix restored, a full translate-only pass over the real file (472,129 lines of Core emitted) completes with zero errors. Also re-ran translate-only passes over several other existing while-loop-bearing fixtures (`mux4_1_concrete.mlir`, `simple_loop_concrete.mlir`, `three_subcomponents_loop_concrete.mlir`, `poseidon3_test_concrete.mlir`, `poseidon3_new.mlir`) to confirm no regression in the already-working "locally redefined constant" shape.
